...
handleProtClause (protSpec,protphrs)
        = map (\p -> protclauseHelper p protSpec) protphrs

-- *************************************************************************

protclauseHelper :: ProtocolPhrase -> ProtocolSpec -> (Name,HandVal)
protclauseHelper (handName,protocol) (protname,types,params) 
        = (handName,(protname,protocol,(length types,length params)))  
...
removeProcDefn :: Defn -> (Name,(Maybe ProcessType,(NumArgs,NumArgs,NumArgs)))
removeProcDefn (ProcessDefn (pname,ptype,plist,_))
    = (pname,(ptype,(l1,l2,l3))) 
        where
            pattProc   = (fst.head) plist
